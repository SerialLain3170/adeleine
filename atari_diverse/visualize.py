...
class Visualizer:
    ...
    def __call__(self,
        ...
        for index in range(testsize):
            self._save(v_list[0][index], width, width*index+1, outdir, epoch)
            self._save(v_list[1][index], width, width*index+2, outdir, epoch)
            self._save(v_list[2][index], width, width*index+3, outdir, epoch)
            self._save(v_list[3][index], width, width*index+4, outdir, epoch)
            self._save(y[index], width, width*index+5, outdir, epoch)
