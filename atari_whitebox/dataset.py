...
class IllustDataset(Dataset):
    ...
    def _preprocess(self,
                    ...
                    ss: np.array,
                    size: int):
        line, color, ss = self._random_crop(line, color, ss, size=size)
    ...
    def valid(self, validsize: int):
        ...
        for index in range(validsize):
            ...
            color, line, mask, _ = self._preprocess(color,
                                                    line,
                                                    line,
    ...
    def __getitem__(self, idx):
        ...
        color, line, mask, ss = self._preprocess(color,
                                                 line,
                                                 ss,
                                                 self.train_size)
        ...
        line = self._coordinate(line, self.line_space)
        mask = self._coordinate(mask, self.color_space)
        ss = self._coordinate(ss, self.color_space)
